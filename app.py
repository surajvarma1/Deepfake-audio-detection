...
def load_audio(audiopath, sampling_rate=22000):
    if isinstance(audiopath, str):
        if audiopath.endswith('.mp3'):
            audio, isr = librosa.load(audiopath, sr=sampling_rate)
    ...
    if uploaded_file is not None:
        if st.button("Analyze Audio"):
            ...
            with col1:
                ...
                st.success(f"The uploaded audio is  likely to be AI Generated.")
            ...
            with col3:
                st.info("Disclaimer")
                st.warning("These classification or detection mechanisms are not always accurate. They should be  consideres as a strong signal and not the ultimate decision maker.")
